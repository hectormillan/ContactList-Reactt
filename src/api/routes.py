...
@api.route("/edit-profile/", methods=["PUT"])  # FIX: El endpoint edit profile es incorrecto. Lo correcto es user/users/<int: id> (REST API FULL)  
# @jwt_required()
def edit_user():
    response_body = {}
...
def user_characters_favourites(user_id):
    response_body = {}
    rows = db.session.execute(db.select(CharactersFavourites).where(CharactersFavourites.user_id == user_id)).scalars()
    if request.method == "GET":
        results = [row.serialize() for row in rows]
    ...
    if request.method == "POST":
        ...
        existing_favourite = db.session.execute(db.select(CharactersFavourites)
                                                  .where(CharactersFavourites.user_id == user_id, CharactersFavourites.character_id == character_id)).scalar_one_or_none()
...
def delete_user_characters_favourites(user_id, character_id):
    response_body = {}
    row = db.session.execute(db.select(CharactersFavourites).where(CharactersFavourites.user_id == user_id, CharactersFavourites.character_id == character_id)).scalar()
    if row:
...
def user_planets_favourites(user_id):
    response_body = {}
    rows = db.session.execute(db.select(PlanetsFavourites).where(PlanetsFavourites.user_id == user_id)).scalars()
    if request.method == "GET":
        results = [row.serialize() for row in rows]
    ...
    if request.method == "POST":
        ...
        existing_favourite = db.session.execute(db.select(PlanetsFavourites).where(PlanetsFavourites.user_id == user_id, PlanetsFavourites.planet_id == planet_id)
                                                 ).scalar_one_or_none()
        if existing_favourite:
...
def delete_user_planets_favourites(user_id, planet_id):
    response_body = {}
    row = db.session.execute(db.select(PlanetsFavourites).where(PlanetsFavourites.user_id == user_id, 
                                                                PlanetsFavourites.planet_id == planet_id)).scalar()
    if row:
        db.session.delete(row)
        db.session.commit()
